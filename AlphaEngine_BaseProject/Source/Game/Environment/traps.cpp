...
Box MakePlayerFeetBox(const Player& p)
{
    ...
    // width covers the whole player to make it easier to step on plates, and
    // also visually looks better since the plate is under the player's feet.
    b.size.x = fullSize.x * 0.4f;

    // thin strip at the bottom for stepping checks
    b.size.y = 0.4f;

    // align to player's bottom area
...
Box MakePlayerBodyBox(const Player& p)
{
    ...
    // traps that damage on touch should use this box
    b.size.x = fullSize.x * 0.1f;
    b.size.y = fullSize.y * 0.55f;
...
// -----------------------------------------------------------------------------
// PressurePlate shared resources
// -----------------------------------------------------------------------------
...
// Trap
// -----------------------------------------------------------------------------
Trap::Trap(Type type, const Box& box)
    : m_type(type), m_box(box)
{
}
...
float TrapManager::GetClosestLavaDistance(const AEVec2& point) const
{
    ...
    for (const auto& t : m_traps)
    {
        ...
        if (t->GetType() != Trap::Type::LavaPool) continue;
...
void LavaPool::OnPlayerEnter(Player& player)
{
    ...
    // immediate hit on enter, then continue periodic damage
    m_tickTimer = 0.f;
