...
- rendering tiles
- setting/getting tiles
- basic tile collision

In short:  
**`MapGrid` is the actual map storage + draw + collision layer.**
...
In short:  
**`LevelEditor` is the gameplay-side controller of the editor. It decides what happens when the user clicks on the map.**
...
In short:  
**`EditorUI` lets the user choose what they want to place, but it does not place anything by itself.**
...
In short:  
**`LevelIO` is what makes editor data persist as level files.**

---

## 2. Full process: how everything connects
...
3. **`EditorUI`** lets the user choose a brush/tool.
4. **`LevelEditor`** reads that brush choice and places tiles / enemies / traps / spawn into runtime editor state.
...
## 3. HOW TO ADD A NEW TILE TYPE

Adding a tile is a multi-file change. Do **not** change only one place.

### Step 1: Add the tile enum in `MapTile.h`
